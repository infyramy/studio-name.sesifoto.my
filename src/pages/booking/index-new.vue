...
const dates = computed(() => {
  ...
  for (const d of dateList) {
    ...
    const isBeforeBookingWindow =
      bookingWindowStart && dateString < bookingWindowStart;

    // Check if date is after booking window end (shouldn't happen, but double-check)
    const isAfterBookingWindow =
      bookingWindowEnd && dateString > bookingWindowEnd;
    ...
    const isBlackout =
      blackoutDates.value.some((blackout) => {
        if (!blackout.end_date) {
          return dateString === blackout.start_date;
        }
        return (
          dateString >= blackout.start_date && dateString <= blackout.end_date
        );
      }) ||
      isStudioClosed ||
    ...
    for (const rule of pricingRules.value) {
      if (
        ...
        if (
          rule.applies_to_themes === "all" ||
          (selectedTheme.value &&
            Array.isArray(rule.applies_to_themes) &&
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-3 mt-2 text-xs text-gray-500">
                ...
                <span class="font-bold text-gray-900"
                  >RM{{ selectedTheme.base_price }}</span
                >
              </div>
            </div>
          </div>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    ></div>
                    <span
                      v-if="d.specialType === 'percentage_increase'"
                      :class="[
                        'text-[8px] font-bold',
                        selectedDate === d.date
                          ? 'text-white'
            ...
            >
              ...
              <div class="text-xs font-sans leading-relaxed flex-1">
                ...
                <div class="space-y-1">
                  ...
                  <p
                    v-if="
                      selectedDateInfo.specialType === 'percentage_increase'
                    "
                  ...
                  >
                    ...
                    <span class="font-bold"
                      >+{{
                        Math.round((selectedDateInfo.priceModifier - 1) * 100)
                      }}%</span
                    >
                    {{ t("fromBasePrice") || "daripada harga asas" }}
                  </p>
                  <p
                    v-else-if="selectedDateInfo.specialType === 'fixed_price'"
                    class="text-amber-800"
                  >
