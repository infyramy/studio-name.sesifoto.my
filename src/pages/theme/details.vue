...
const nextImage = () => {
  ...
  currentImageIndex.value = (currentImageIndex.value + 1) % theme.value.images.length;
};

const prevImage = () => {
  if (!theme.value) return;
  currentImageIndex.value =
    currentImageIndex.value === 0 ? theme.value.images.length - 1 : currentImageIndex.value - 1;
};
  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    ...
    <header class="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-200">
      <div class="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          ...
          />
          <span class="text-sm font-semibold text-gray-900">{{ studioStore.studio?.name }}</span>
        </div>
      </div>
    </header>
    ...
    <div v-else-if="theme" class="container mx-auto px-4 py-8 max-w-6xl">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        <!-- Image Carousel -->
        <div class="fade-in">
          <div class="relative aspect-[4/3] rounded-2xl overflow-hidden bg-gray-200 shadow-xl">
            <img
              :src="currentImage"
              :alt="theme.name"
            ...
            <!-- Navigation Arrows -->
            <div v-if="theme.images.length > 1" class="absolute inset-0 flex items-center justify-between p-4">
              <button
                @click="prevImage"
            ...
            <!-- Image Indicators -->
            <div v-if="theme.images.length > 1" class="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
              <button
        ...
        <div class="slide-up space-y-6">
          ...
          <div class="p-4 bg-blue-50 rounded-xl border border-blue-100">
            <p class="text-gray-700 leading-relaxed">{{ theme.description_long }}</p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="p-4 bg-white rounded-xl shadow-soft">
              ...
              <p class="text-2xl font-bold text-gray-900">{{ theme.base_pax }} orang</p>
              <p class="text-xs text-gray-500 mt-1">+RM{{ theme.extra_pax_price }}/orang tambahan</p>
            </div>
            ...
            <div class="p-4 bg-white rounded-xl shadow-soft">
              ...
              <p class="text-2xl font-bold text-gray-900">{{ theme.duration_minutes }} minit</p>
              <p class="text-xs text-gray-500 mt-1">Sesi fotografi</p>
            </div>
          </div>

          <!-- Pricing -->
          <div class="p-6 bg-gradient-to-br from-primary-50 to-primary-100 rounded-2xl border border-primary-200">
            <div class="flex items-baseline justify-between mb-4">
              <div>
                <p class="text-sm text-gray-600 mb-1">Harga bermula dari</p>
                <p class="text-4xl font-bold text-primary-700">RM{{ theme.base_price }}</p>
                <p v-if="theme.is_deposit" class="text-sm text-gray-600 mt-1">This amount is a deposit</p>
              </div>
            </div>
